...
public class SubNode extends OpNode {
    ...
    public void executor(Executor executor) throws VMRuntimeException {
        ...
        if(t1.getType()==ExObject.STRING||t2.getType()==ExObject.STRING||t1.getType()==ExObject.BOOLEAN||t2.getType()==ExObject.BOOLEAN)throw new VMRuntimeException("字符串或布尔值类型不能参加减法运算",executor.getThread());
        ...
        else if(t1.getType()==ExObject.NULL||t2.getType()==ExObject.NULL)throw new VMRuntimeException("减法运算时发生空指针异常",executor.getThread());
        else{
