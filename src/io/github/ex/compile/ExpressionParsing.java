...
import io.github.ex.exe.code.opcode.*;
import io.github.ex.exe.code.struct.GroupASTNode;
import io.github.ex.exe.code.struct.InvokeASTNode;
import io.github.ex.exe.code.struct.MovVarNode;
import io.github.ex.exe.obj.*;
import io.github.ex.compile.parser.InvokeParser;
        ...
        while (true) {
            ...
            try {
                ...
                if (isOperator(token)) {
                    ...
                    else if(compiler.array_names.contains(token.getData())||compiler.getValueNames().contains(token.getData())){
                        Token name = token;
